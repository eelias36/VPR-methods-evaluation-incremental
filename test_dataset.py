...
class TestDataset(data.Dataset):
    def __init__(self, img_folder, angle_threshold=5, image_size=None, use_labels=True):
        """Dataset with images from database and queries, used for validation and test.
        Parameters
        ----------
        dataset_folder : str, should contain the path to the test set,
            which contains the folder {img_folder}.
        img_folder : str, name of folder with the database.
        angle_threshold : int, angle in degrees for a prediction to be considered a positive.
        """
        super().__init__()

        self.images_paths = read_images_paths(img_folder)

        # Get order of images using filename
